...
class NewsRequest(BaseModel):
    query: str
    limit: Optional[int] = 12  # Default to 9 articles (3 from each leaning)
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            for query_type, query in enumerate(queries):
                ...
                try:
                    ...
                    print(
                        f"Extracted {len(articles)} articles for {political_leaning}-leaning sources"
                    )
            ...
            stats = {
                ...
                "left_count": sum(
                    1 for s in balanced_sources if s.political_leaning == "left"
                ),
                "center_count": sum(
                    1 for s in balanced_sources if s.political_leaning == "center"
                ),
                "right_count": sum(
                    1 for s in balanced_sources if s.political_leaning == "right"
                ),
            }
            ...
            political_scores = [
                s.political_score
                for s in balanced_sources
                if s.political_score is not None
            ]
